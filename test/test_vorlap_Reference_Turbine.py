...
natfreqs = np.loadtxt(f"{path}/../data/natural_frequencies_Reference_Turbine.csv", delimiter=',')
...
components = vorlap.load_components_from_csv(f"{path}/../data/components/componentsHVAWTReference/")
...
for iInflow in range(viv_params.inflow_speeds.size):
       for jAzi in range(viv_params.azimuths.size):
              print(f"V: {viv_params.inflow_speeds[iInflow]}, Azi: {viv_params.azimuths[jAzi]}, Info: {percdiff_info[iInflow,jAzi]}")
...
plt.ylabel('Inflow (m/s)')
plt.title('Fx ')
plt.savefig(f"{figs_dir}/Fx_optimized.pdf", bbox_inches='tight',transparent=True)
plt.close()
...
plt.ylabel('Inflow (m/s)')
plt.title('Fy ')
plt.savefig(f"{figs_dir}/Fy_optimized.pdf", bbox_inches='tight',transparent=True)
plt.close()
...
plt.ylabel('Inflow (m/s)')
plt.title('Fz ')
plt.savefig(f"{figs_dir}/Fz_optimized.pdf", bbox_inches='tight',transparent=True)
plt.close()
...
plt.ylabel('Inflow (m/s)')
plt.title('Mx ')
plt.savefig(f"{figs_dir}/Mx_optimized.pdf", bbox_inches='tight',transparent=True)
plt.close()
...
plt.ylabel('Inflow (m/s)')
plt.title('My ')
plt.savefig(f"{figs_dir}/My_optimized.pdf", bbox_inches='tight',transparent=True)
plt.close()
...
# plt.ylabel('Inflow (m/s)')
# plt.title('Mz ')
# plt.savefig(f"{figs_dir}/Mz_optimized.pdf", bbox_inches='tight', transparent=True)
# plt.close()
...
plt.imshow(
    ...
    aspect='auto',
    origin='lower',
    cmap='coolwarm',      # blue-grey-red diverging
    norm=norm             # ensures 0 maps to grey midpoint
)
plt.colorbar(label='Moment (N·m)')
plt.xlabel('Azimuth (deg)')
plt.ylabel('Inflow (m/s)')
...
df = pd.read_csv(f"{path}/forces_output_optimized.csv", skipinitialspace=True)

# --- Plot ---
plt.figure(figsize=(4.5, 3))
plt.plot(df["time"], df["node2x"], label="Node 2 X", linewidth=2)
plt.plot(df["time"], df["node2y"], label="Node 2 Y", linewidth=2)
...
# plt.grid(True)
plt.tight_layout()
plt.savefig(f"{figs_dir}/ReconstructedForce.pdf", bbox_inches='tight', transparent=True)
plt.close()
